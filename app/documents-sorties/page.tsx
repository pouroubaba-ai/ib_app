...
export default function DocumentsSortiesPage() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    async function load() {
      const [snap, retourSnap] = await Promise.all([
        getDocs(query(collection(db, 'documents_stock'), where('userId', '==', user!.uid), where('typeDocument', '==', 'Sortie'))),
    ...
    <AppLayout>
      <div className="max-w-7xl mx-auto">
        <div className="mb-6 flex items-start justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Documents de sorties</h1>
            <p className="text-gray-500 text-sm mt-1">{filtered.length} document(s)</p>
          </div>
          <div className="text-right">
            <p className="text-xs text-gray-400">Total</p>
            <p className="text-lg font-bold text-blue-600">{formatMontant(total)}</p>
          </div>
        </div>
        ...
        <div className="relative mb-4 max-w-sm">
          ...
          <input
            type="text"
            placeholder="Rechercher un client ou N° document..."
            value={recherche}
                ...
                <div key={d.id} onClick={() => router.push(`/document/${d.id}`)}
                  ...
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <span className={`text-xs font-medium px-1.5 py-0.5 rounded-full shrink-0 ${d.livraison === 'livre' ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700'}`}>{d.livraison === 'livre' ? 'Terminé' : 'En cours'}</span>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {filtered.length === 0 ? (
                      <tr><td colSpan={8} className="text-center py-10 text-gray-400">Aucun document</td></tr>
                    ) : filtered.map(d => (
                      <tr key={d.id} onClick={() => router.push(`/document/${d.id}`)} className="cursor-pointer hover:bg-indigo-50/40 transition-colors">
                        ...
                        <td className="px-4 py-3 text-right font-semibold text-blue-600">{formatMontant(d.totalGeneral)}</td>
                        <td className="px-4 py-3 text-center">
                          <span className={`px-2 py-1 rounded-full text-xs font-medium ${d.livraison === 'livre' ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700'}`}>{d.livraison === 'livre' ? 'Terminé' : 'En cours'}</span>
                        </td>
                        <td className="px-4 py-3 text-center">
