...
export default function HistoriqueSortiesPage() {
  const { user } = useAuth();
    ...
    <AppLayout>
      <div className="max-w-7xl mx-auto">
        <div className="mb-6 flex items-start justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Historique des sorties</h1>
            <p className="text-gray-500 text-sm mt-1">
              {filtered.length} mouvement(s)
              {filtreActif && <span className="ml-2 text-blue-600 font-medium">· {formatMontant(totalFiltre)}</span>}
            </p>
          </div>
          <div className="text-right">
            <p className="text-xs text-gray-400">Total général</p>
            <p className="text-lg font-bold text-blue-600">{formatMontant(totalGeneral)}</p>
          </div>
        </div>
        ...
        {/* Recherche produit + filtre client */}
                  ...
                  {personnesFiltrees.length === 0 ? (
                    ...
                    <button key={p} onClick={() => { setFiltrePersonne(filtrePersonne === p ? '' : p); setRecherchePanel(''); setShowPanel(false); }}
                      className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-colors
                        ${filtrePersonne === p ? 'bg-blue-600 text-white' : 'hover:bg-gray-100 text-gray-700'}`}>
                      {p}
                    </button>
                  ))}
                </div>
        ...
        {filtrePersonne && (
          <div className="flex items-center gap-2 mb-3">
            <span className="flex items-center gap-1.5 bg-blue-50 text-blue-700 text-xs font-medium px-3 py-1.5 rounded-full border border-blue-200">
              {filtrePersonne}
            ...
            <div className="sm:hidden space-y-2">
              ...
              {filtered.length > 0 && (
                <div className="bg-blue-50 rounded-xl border border-blue-100 p-4 flex justify-between items-center">
                  ...
                  <span className="font-bold text-blue-600">{formatMontant(totalFiltre)}</span>
                </div>
              )}
            </div>
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {filtered.length === 0 ? (
                      ...
                      <tr key={m.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-4 py-3 text-right font-semibold text-blue-600">{formatMontant(m.totalLigne)}</td>
