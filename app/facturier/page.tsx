...
export default function FacturierDocumentsPage() {
  ...
  const [onglet, setOnglet] = useState<'en_cours' | 'termine'>('en_cours');
      ...
      const data: DocSortie[] = docSnap.docs
        ...
        .filter(d => {
          ...
          const traite = d.facturierTraites.length >= d.nombreDeProduit && d.nombreDeProduit > 0
            && !d.facturierPendingRetour;
          // Aujourd'hui: tout afficher. Jours passés: uniquement non traités ou retour en attente.
  ...
  const liste = onglet === 'en_cours' ? enCours : termine;
  ...
  return (
    <FacturierLayout>
      ...
      <div className="px-4 pt-5 pb-4">
        <div className="mb-4">
          <p className="font-bold text-gray-900 text-lg">Documents du jour</p>
        ...
        {loading ? (
          ...
          <div className="text-center py-16 text-gray-400">
            ...
            <p className="text-sm">{onglet === 'en_cours' ? 'Tout est traité !' : 'Aucun document terminé'}</p>
          </div>
        ) : (
              ...
              return (
                <div key={d.id} onClick={() => router.push(`/facturier/document/${d.id}`)}
                  className={`bg-white rounded-2xl border shadow-sm p-4 cursor-pointer active:bg-gray-50
                    ${pendingRetour ? 'border-orange-400' :
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        {pendingRetour && (
                          <button
                            ...
                            className="shrink-0 p-0.5 rounded-full hover:bg-orange-100 transition-colors"
                          >
